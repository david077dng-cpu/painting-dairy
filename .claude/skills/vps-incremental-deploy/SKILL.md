...
description: 增量部署 Astro 项目到 VPS (root@47.77.237.222 xlilian.art)。当用户需要增量部署、直接部署到VPS、检查VPS部署状态、回滚部署时使用这个skill。包括预检查、本地构建、rsync增量同步、自动备份、健康检查全流程自动化。
---

# VPS 增量部署 Skill

增量部署本地 Astro 项目到 `root@47.77.237.222` VPS 的自动化 skill。针对 `xlilian.art` 网站部署优化。
...
- 需要检查当前 VPS 上的部署状态时
- 需要回滚到之前的部署版本时
- 提到 `47.77.237.222` 或 `xlilian.art` 部署相关操作时
...
```bash
# 连接 VPS 并报告当前状态
ssh root@47.77.237.222 "echo '=== Deployment Status ===' && ls -lh /var/www/html/ && echo && echo '=== Nginx Config ===' && cat /etc/nginx/sites-enabled/*"
```
...
| 配置项 | 值 |
|--------|-----|
| VPS Host | `root@47.77.237.222` |
| 远程目录 | `/var/www/html` |
| 本地构建目录 | `./dist` |
| 备份目录 | `/var/www/backups` |
| 网站 URL | `https://xlilian.art` |
